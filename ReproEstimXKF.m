...
for i=1:((tf-1)/dt)
    ...
    FX    = [1-gamma*xbar(4)*xbar(2)*dt/N -gamma*xbar(4)*xbar(1)*dt/N 0 -gamma*xbar(1)*xbar(2)*dt/N;
             gamma*xbar(4)*xbar(2)*dt/N 1+gamma*xbar(4)*xbar(1)*dt/N-gamma*dt 0 gamma*xbar(1)*xbar(2)*dt/N;
             0 gamma*dt 1 0;
             0 0 0 1];
         
%    xhat = FX*(xhat-xbar)+xbar;
         
    Pmin  = FX*Pplus*FX'+QF;
    % update 
    KF    = Pmin*C'*inv(C*Pmin*C'+RF);
    xhat  = FX*(xhat-xbar)+xbar + KF*(y(:,i)-C*xhat);
...
grid on
grid minor
legend('XKF','mean XKF','Cori et al.','mean Cori et al.')